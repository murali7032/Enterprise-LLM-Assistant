...
async def test_playbook_runs_when_approved() -> None:
  tool = K8sPlaybookTool(client=KubernetesClient())
  result = await tool.execute(json.dumps({"action": "restart", "name": "my-app", "approved": True}))
...
async def test_agent_playbook_pending_approval(monkeypatch) -> None:
  class StubPlanner:
    async def plan(self, goal, observations, tools):
      return {"thought": "restart it", "action": "k8s_playbook", "input": "restart deploy demo"}

  approvals = ApprovalStore()
  client = KubernetesClient()
  router = ToolRouter(tools=[WeatherTool(), KubernetesTool(client=client), K8sPlaybookTool(client=client)])
