...
@pytest.mark.asyncio
async def test_conversation_memory_char_trim() -> None:
    memory = InMemoryConversationMemory()
    await memory.append("s1", "user", "a" * 100)
    await memory.append("s1", "assistant", "b" * 100)
    await memory.append("s1", "user", "recent question")

    history = await memory.get_history("s1", limit=10, max_chars=120)
...
async def test_chat_service_uses_memory_in_prompt(monkeypatch) -> None:
    ...
    memory = InMemoryConversationMemory()
    await memory.append("session-1", "user", "My favorite color is blue")
    await memory.append("session-1", "assistant", "Noted.")
    ...
    assert len(await memory.get_history("session-1")) == 4
...
async def test_chat_service_disables_memory_when_requested(monkeypatch) -> None:
    monkeypatch.setattr("app.services.chat_service.settings.MEMORY_ENABLED", True)
    memory = InMemoryConversationMemory()
    await memory.append("session-1", "user", "Secret context")
    ...
    assert len(await memory.get_history("session-1")) == 1
