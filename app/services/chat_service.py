...
class ChatService:
    ...
    async def _build_prompt(
        ...
        history = await self._get_history(session_id, request.use_memory)
    ...
    async def _save_exchange(
        ...
        if not use_memory or not settings.MEMORY_ENABLED:
            return
        await self._memory.append(session_id, "user", question)
        await self._memory.append(session_id, "assistant", answer)
    ...
    async def chat(self, request: ChatRequest) -> ChatResponse:
        ...
        use_rag = (
            request.use_rag
            or bool(request.metadata_filter)
            or self._detect_intent(request.question) == "rag"
        )
